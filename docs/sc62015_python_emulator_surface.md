...
2. **Reset path**: `power_on_reset()` delegates to `intrinsics.eval_intrinsic_reset` and reads the three-byte reset vector at `0xFFFFD` (the interrupt vector is separately stored at `0xFFFFA`). External users often pass `reset_on_init=False` and call `power_on_reset` manually when they need fine-grained control. The remaining RESET register side effects are model contracts pending dedicated hardware tracing.
...
4. **Preflight**: device wrappers call `CPU.validate_before_scheduling()` before advancing timers or peripherals. Reserved/noncanonical encodings, `TCL`, and every `I=0` counted instruction are rejected there and again inside the selected backend.
