...
class SC62015BaseView(BinaryView):
    ...
    // +8: Drive name follows (variable length, null-terminated, max 5 bytes)
        ...
        try:
            types_result = self.platform.parse_types_from_source(
                iocs_attr_c + "\n" + iocs_entry_c
            )
...
class SC62015FullView(SC62015BaseView):
    ...
    def is_valid_for_data(cls, data) -> bool:
        buf = data.read(0, 4 * 3)
        return (
            buf == b"\x01\x01\x01\x01\x01\x01\x01\x01\x06\x60\x00\x34"
            or buf == b"\x00\x00\x0c\x18\xf8\x0d\x40\x00\x24\x07\x00\x00"
        )
