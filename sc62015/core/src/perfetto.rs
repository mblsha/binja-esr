...
pub enum PerfettoLayout {
    ...
    PythonCompat,
    /// UI-oriented per-call traces for the web Function Runner.
    CallUi,
}
...
pub struct PerfettoTracer {
    ...
    cpu_track: TrackId,
    // Call UI tracks (used when `layout == CallUi`).
...
impl PerfettoTracer {
    ...
    pub fn record_regs(
        ...
        if self.layout == PerfettoLayout::CallUi {
            ...
            let ts_end = ts_start.saturating_add(self.units_per_instr as i64);
            ...
            {
                let mut ev = self.builder.begin_slice(self.instructions_track, name, ts_start);
                ev.add_annotations([
