    ...
    fn write_byte(&self, address: u32, value: u8) {
        Python::with_gil(|py| {
            let _ = self
                .memory
                .bind(py)
                .call_method1("write_byte", (address, value));
        });
    }
}
...
impl LlamaBus for LlamaPyBus {
    fn load(&mut self, addr: u32, bits: u8) -> u32 {
        match bits {
            0 | 8 => self.read_byte(addr) as u32,
            16 => {
                let lo = self.read_byte(addr) as u32;
                let hi = self.read_byte(addr.wrapping_add(1)) as u32;
                lo | (hi << 8)
            }
            24 => {
            ...
            _ => {
                ...
                let mut value = 0u32;
                for i in 0..bytes {
                    value |= (self.read_byte(addr.wrapping_add(i as u32)) as u32) << (8 * i);
                }
                if bits >= 32 {
                    value
                } else {
