...
The web debugger (`web/`) provides an interactive browser interface with advanced debugging features:

### Running the Web Debugger
```bash
cd web
...
#### 1. Instruction History
- Displays last 100 executed instructions in reverse chronological order
- Shows PC address and disassembled instruction text
- Automatically updates during emulation
- Implementation: Rolling buffer using `collections.deque(maxlen=100)`

#### 2. Interactive LCD Debugger
- Hover over any pixel on the LCD display to see which PC last wrote to it
- Yellow column highlight shows the 1x8 pixel group under cursor
- Tracks PC source for each VRAM byte in HD61202 chips
- Useful for reverse engineering graphics rendering routines

#### 3. Internal Register Watch
- Monitors access to SC62015 internal memory-mapped registers (IMEMRegisters)
- Tracks last 10 PC addresses for reads and writes to each register
- Hover over register names for detailed descriptions
- Covers keyboard I/O (KOL/KOH/KIL), UART, interrupts, system control

#### 4. Click-to-Copy PC Addresses
- All PC addresses in the UI are clickable for easy copying
- Visual feedback with "Copied!" message
- Works in CPU state, instruction history, register watch, and LCD tooltips
- Implemented via reusable `PCAddress` JavaScript component

### API Endpoints

- `GET /api/v1/state` - Full emulator state including instruction history
- `GET /api/v1/lcd_debug?x=X&y=Y` - Get PC source for LCD pixel
- `GET /api/v1/imem_watch` - Get internal register access tracking
- `POST /api/v1/key` - Send keyboard input
- `POST /api/v1/control` - Control emulation (run/pause/step/reset)

### Implementation Notes

#### Performance Considerations
- Instruction history: O(1) append with automatic size limit
- LCD PC tracking: One value per VRAM byte (minimal overhead)
- Register tracking: Limited to 10 entries per register/operation
- UI polling: 100ms for main state, 500ms for register watch

#### Key Implementation Files
- `pce500/emulator.py`: Added `instruction_history` deque
- `pce500/memory.py`: Added `imem_access_tracking` for register monitoring
- `pce500/display/hd61202.py`: Added `vram_pc_source` tracking
- `web/app.py`: Flask backend with new API endpoints
- `web/static/app.js`: PCAddress component and UI updates

#### Debugging Workflow
1. Use instruction history to understand recent execution flow
2. Hover over LCD to trace graphics rendering code
3. Monitor register watch to debug hardware interactions
4. Click any PC address to copy for breakpoints or searches