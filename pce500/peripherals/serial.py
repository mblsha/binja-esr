...
class SerialQueuedByte:
    """Represents a serialized receive entry."""
...
class SerialSnapshot:
    """Snapshot of UART registers and workspace bytes."""
...
class SerialAdapter:
    """Utility wrapper for serial workspace and IMEM registers."""
    ...
    def handle_imem_access(
        ...
        """Observe TXD writes so transmit bytes can be tracked."""
    ...
    def snapshot(self) -> SerialSnapshot:
        """Capture UART state, handshake byte, and queues."""
    ...
    def restore(self, snapshot: SerialSnapshot) -> None:
        """Restore UART state previously captured via snapshot()."""
    ...
    def consume_received(self) -> Optional[SerialQueuedByte]:
        """Pop the next queued receive byte (simulates RXD read)."""
    ...
    def pending_receive(self) -> List[SerialQueuedByte]:
        """Return snapshot of the receive queue."""
    ...
    def pending_transmit(self) -> List[int]:
        """Return buffered transmit bytes."""

        return list(self._tx_queue)

    def complete_transmit(self) -> Optional[int]:
        """Simulate hardware finishing a transmission."""
    ...
    def _latch_next_received(self) -> None:
        usr = self._read_register(IMEMRegisters.USR)
        usr &= ~(
            self._FRAMING_ERROR_MASK
            | self._OVERRUN_ERROR_MASK
            | self._PARITY_ERROR_MASK
        )
