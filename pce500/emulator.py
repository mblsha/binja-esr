...
class PCE500Emulator:
    ...
    def get_cpu_state(self) -> Dict[str, Any]:
        return {
            'pc': self.cpu.regs.get(RegisterName.PC), 'a': self.cpu.regs.get(RegisterName.A),
            'b': self.cpu.regs.get(RegisterName.B), 'ba': self.cpu.regs.get(RegisterName.BA),
            'i': self.cpu.regs.get(RegisterName.I), 'x': self.cpu.regs.get(RegisterName.X),
            'y': self.cpu.regs.get(RegisterName.Y), 'u': self.cpu.regs.get(RegisterName.U),
            's': self.cpu.regs.get(RegisterName.S),
            'flags': {'z': self.cpu.regs.get(RegisterName.FZ), 'c': self.cpu.regs.get(RegisterName.FC)},
    ...
    def get_performance_stats(self) -> Dict[str, float]:
        ...
        return {'instructions_executed': self.cycle_count, 'elapsed_time': elapsed, 'instructions_per_second': ips, 'speed_ratio': ips / 2_000_000}
    ...
    def _add_register_annotations(self, event: Any):
        ...
        event.add_annotations({
            "reg_A": f"0x{state['a']:02X}", "reg_B": f"0x{state['b']:02X}",
            "reg_BA": f"0x{state['ba']:04X}", "reg_I": f"0x{state['i']:04X}",
            "reg_X": f"0x{state['x']:06X}", "reg_Y": f"0x{state['y']:06X}",
            "reg_U": f"0x{state['u']:06X}", "reg_S": f"0x{state['s']:06X}",
            "reg_PC": f"0x{state['pc']:06X}", "flag_C": state['flags']['c'], "flag_Z": state['flags']['z']
        })

    def _trace_execution(self, pc: int, opcode: Optional[int]):
        event = g_tracer.trace_instant("Execution", f"Exec@0x{pc:06X}", {"pc": f"0x{pc:06X}", "opcode": f"0x{opcode:02X}"})
    ...
    def _update_perfetto_counters(self):
        ...
        g_tracer.trace_counter("CPU", "stack_pointer", self.cpu.regs.get(RegisterName.S))
    ...
    def _trace_control_flow(self, pc_before: int, eval_info):
        ...
        if isinstance(instr, CALL):
            ...
            event = g_tracer.trace_instant("CPU", "return", {"at": f"0x{pc_before:06X}", "type": instr_name.lower(), "depth": self.call_depth})
            self._add_register_annotations(event)
            ...
            event = g_tracer.begin_function("CPU", vector_addr, pc_before, f"int_0x{vector_addr:05X}")
            self.call_depth += 1
            self._add_register_annotations(event)
            g_tracer.trace_instant("CPU", "interrupt", {"from": f"0x{pc_before:06X}", "vector": f"0x{vector_addr:05X}", "interrupt_id": interrupt_id})
            ...
            if pc_after != expected_pc:
                ...
                trace_data = {"from": f"0x{pc_before:06X}", "to": f"0x{pc_after:06X}", "type": "unconditional" if not condition else "conditional_taken"}
    ...
    def _track_imem_access(self, offset: int, access_type: str, cpu_pc: Optional[int]):
        ...
        if reg_name:
            tracking = self.memory.imem_access_tracking.setdefault(reg_name, {"reads": [], "writes": []})
            ...
            if access_list and access_list[-1][0] == cpu_pc:
                ...
                if len(access_list) > 10: access_list.pop(0)
    ...
    def _dump_internal_memory(self, pc: int):
        ...
        g_tracer.trace_instant("Debug", "InternalMemoryDump", {
            "pc": f"0x{pc:06X}", "filename": filename, "size": str(len(internal_mem)),
