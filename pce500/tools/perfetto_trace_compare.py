...
class PerfettoTraceComparator:
    ...
    def extract_execution_events(
        ...
        if execution_uuid is None:
            ...
            track_samples = {}
            for packet in trace.packet:
                if packet.HasField("track_event") and packet.track_event.HasField(
                    "name"
                ):
            ...
            for uuid, samples in track_samples.items():
                if any(name.startswith("Exec@") for name in samples):
                    execution_uuid = uuid
                    print(
                        f"  Note: Found Exec@ events on track {uuid} (thread name: '{track_names.get(uuid, 'unnamed')}')"
                    )
                    break
    ...
    def compare_events(
        ...
        Tuple[int, Optional[ExecutionEvent], Optional[ExecutionEvent], Optional[str]]
    ]:
        """
        ...
        for i in range(max_len):
            ...
            if event1.name != event2.name:
                yield (
                    ...
                    event1,
                    event2,
                    f"Event name mismatch: '{event1.name}' vs '{event2.name}'",
                )
                return
            ...
            if error:
                ...
                for j in range(i - context_size, i + 1):
                    if j >= 0 and j < len(self.trace1_events):
                        ...
                        pc2 = (
                            self.trace2_events[j].annotations.get("pc", "N/A")
                            if j < len(self.trace2_events)
                            else "N/A"
                        )
                        op1 = self.trace1_events[j].annotations.get("opcode", "N/A")
                        op2 = (
                            self.trace2_events[j].annotations.get("opcode", "N/A")
                            if j < len(self.trace2_events)
                            else "N/A"
                        )
                        marker = " -> " if j == i else "    "
                        ...
                        op_match = op1 == op2
                        print(
                            f"   {marker}Event #{j}: PC={pc1} opcode={op1} {'✓' if pc_match and op_match else '✗' if not pc_match or not op_match else ''}"
                        )
...
def main():
    ...
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="Show progress during comparison"
    )
