...
def demonstrate_keyboard_strobing():
    ...
    program = bytes(
        [
            # Scan column 0
            0x08,
            0xFE,  # MV A, 0xFE (KO0 low, others high)
            0xA0,
            0xF0,  # MV (KOL), A
            0x08,
            0xFF,  # MV A, 0xFF (all KOH high)
            0xA0,
            0xF1,  # MV (KOH), A
            0x80,
            0xF2,  # MV A, (KIL) - read keyboard
            0xC0,
            0x00,  # MV (0x00), A - store result
            # Scan column 1
            0x08,
            0xFD,  # MV A, 0xFD (KO1 low, others high)
            0xA0,
            0xF0,  # MV (KOL), A
            0x80,
            0xF2,  # MV A, (KIL)
            ...
            0x08,
            0xFF,  # MV A, 0xFF (all KOL high)
            0xA0,
            0xF0,  # MV (KOL), A
            0x08,
            0xFB,  # MV A, 0xFB (KO10 low - bit 2)
            0xA0,
            0xF1,  # MV (KOH), A
            0x80,
            0xF2,  # MV A, (KIL)
    ...
    print(
        f"Column 1 scan result: 0x{col1_result:02X} (expected 0xFF - no key in this column)"
    )
    ...
    col0_result = emu.memory.read_byte(0x00)
    print(
        f"Column 0 scan with KSD enabled: 0x{col0_result:02X} (expected 0xFF - keyboard disabled)"
    )
