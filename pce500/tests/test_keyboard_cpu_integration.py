...
class TestKeyboardCPUIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # This program:
        # 1. Sets KOL to 0x01 (select column 0 with active-high logic)
        # 2. Reads KIL to register A
        # 3. Loops forever
        test_program = bytes(
            [
                # MV A, 0x01   - Load 0x01 into A (bit 0 high = column 0 active)
                0x08,
                0x01,
    ...
    def test_keyboard_register_access(self):
        ...
        # Verify KOL was set
        kol_value = self.emulator.memory.read_byte(
            INTERNAL_MEMORY_START + IMEMRegisters.KOL
        )
        self.assertEqual(kol_value, 0x01)

        # No keys pressed, so KIL should read 0x00 (all bits low with active-high logic)
    ...
    def test_multiple_rows(self):
        ...
        # MV A, 0x02   - Load 0x02 into A (bit 1 high = column 1 active)
