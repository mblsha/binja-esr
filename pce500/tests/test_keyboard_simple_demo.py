...
def test_keyboard_strobing():
    ...
    kil = keyboard.read_register(0xF2)
    print(f"Strobe columns 0&1, read KIL: 0x{kil:02X} (expected 0xFD - both keys on row 1)")
    ...
    kil = keyboard.read_register(0xF2)
    print(f"With KSD enabled, read KIL: 0x{kil:02X} (expected 0xFF - keyboard disabled)")
    
    # Disable KSD
    memory_data[0x1000FE] = 0x00
    kil = keyboard.read_register(0xF2)
    print(f"With KSD disabled, read KIL: 0x{kil:02X} (expected 0xFE - key detected again)")
