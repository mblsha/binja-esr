...
class TestSimplifiedEmulator:
    ...
    def test_rom_loading(self):
        ...
        # Verify ROM is loaded at correct address (0xC0000)
    ...
    def test_reset_functionality(self):
        ...
        # ROM starts at 0xC0000, so offset is 0xFFFFD - 0xC0000 = 0x3FFFD
    ...
    def test_mv_bp_register_instruction_execution(self):
        ...
        for i, expected_byte in enumerate(opcode_bytes):
            ...
            assert actual_byte == expected_byte, (
                f"ROM byte {i}: expected 0x{expected_byte:02X}, got 0x{actual_byte:02X}"
            )
        ...
        assert initial_bp_via_memory == initial_bp_via_cpu, (
            f"Both methods should give same initial value: memory={initial_bp_via_memory:02X}, cpu={initial_bp_via_cpu:02X}"
        )
        ...
        assert result is True, (
            "Instruction execution should continue (not hit breakpoint)"
        )

        # Verify PC advanced by instruction length (4 bytes)
        final_pc = emu.cpu.regs.get(RegisterName.PC)
        assert final_pc == initial_pc + 4, (
            f"PC should advance by 4 bytes: expected 0x{initial_pc + 4:06X}, got 0x{final_pc:06X}"
        )
        ...
        assert final_bp_via_memory == final_bp_via_cpu, (
            f"Both methods should give same final value: memory={final_bp_via_memory:02X}, cpu={final_bp_via_cpu:02X}"
        )

        # Verify that BP value correctly changed to 0xC2 (the immediate value from the MV instruction)
        assert final_bp_via_memory == 0xC2, (
            f"BP should be 0xC2 after MV (EC), C2 instruction, got 0x{final_bp_via_memory:02X}"
        )
        assert final_bp_via_cpu == 0xC2, (
            f"BP should be 0xC2 via cpu.memory access, got 0x{final_bp_via_cpu:02X}"
        )

        # Verify memory operations occurred
        assert emu.memory_read_count > initial_reads, (
            "Should have read memory during instruction fetch"
        )
        assert emu.memory_write_count > initial_writes, (
            "Should have attempted memory write for MV instruction"
        )
    ...
    def test_mv_bp_register_with_sc62015_direct(self):
        ...
        assert final_bp == 0xC2, (
            f"BP should be 0xC2 after instruction, got 0x{final_bp:02X}"
        )
