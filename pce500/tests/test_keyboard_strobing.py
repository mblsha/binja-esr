...
class TestKeyboardStrobing:
    ...
    def test_basic_key_press_with_strobing(self):
        ...
        assert kil == 0b11111101, (
            "KI1 should be low when Q is pressed and KO0 is strobed"
        )
    ...
    def test_koh_column_strobing(self):
        ...
        kil = keyboard.read_register(KIL)
        assert kil == 0b11111110, (
            "KI0 should be low when P is pressed and KO10 is strobed"
        )
    ...
    def test_realistic_scanning_sequence(self):
        ...
        for col in range(11):
            ...
            if col == 6:  # Column for '5' key
                assert kil == 0b11011111, (
                    f"KI5 should be low when scanning column {col}"
                )
            else:
                assert kil == 0xFF, (
                    f"No key should be detected when scanning column {col}"
                )
