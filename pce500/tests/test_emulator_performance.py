...
class TestEmulatorPerformance:
    ...
    def test_emulator_boot_performance(self):
        ...
        for stat in stats:
            chip_name = "Left" if stat["chip"] == 0 else "Right"
            assert stat["instructions"] > 0, (
                f"{chip_name} chip received no instructions"
            )
            assert stat["on_off_commands"] > 0, (
                f"{chip_name} chip received no ON/OFF commands"
            )
        ...
        total_cs = (
            emu.lcd.cs_both_count + emu.lcd.cs_left_count + emu.lcd.cs_right_count
        )
        ...
        assert emu.instruction_count == 20000, (
            f"Expected 20000 instructions, got {emu.instruction_count}"
        )
        ...
        for stat in stats:
            ...
            assert stat["data_written"] >= 0, (
                f"{chip_name} chip has negative data_written"
            )
