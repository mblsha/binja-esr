...
def ocr_image(
    ...
    if save_preprocessed:
        preprocessed_path = f"{output_prefix}_psm{psm_mode}_th{threshold}_sc{scale_factor}.png"
...
def test_ocr_configurations():
    ...
    if not Path(test_image).exists():
        print(f"Test image {test_image} not found. Please run the emulator first to generate it.")
        return
    ...
    configurations.extend([
        # No preprocessing
        {"psm_mode": 6, "threshold": None, "scale_factor": 1.0, "invert": False, "border_size": 0},
        # Adaptive threshold
        {"psm_mode": 6, "threshold": 128, "scale_factor": 2.0, "adaptive_threshold": True},
        # With denoising
    ...
    for i, config in enumerate(configurations, 1):
        ...
        print(f"  PSM={config.get('psm_mode', 6)}, "
        ...
        try:
            ...
            for j, (expected, recognized) in enumerate(zip(expected_lines, recognized_lines + [''] * 4), 1):
            ...
            print(f"  Total Score: {total_score:.1f}% (total distance={total_distance})")
...
def test_single_configuration():
    ...
    for i, (expected, recognized) in enumerate(zip(expected_lines, recognized_lines + [''] * 4), 1):
        score = similarity_score(expected, recognized)
        if score == 100:
...
def test_best_configurations():
    ...
    best_configs = [
        ...
        {"psm_mode": 3, "threshold": 128, "scale_factor": 4.0, "border_size": 5, "invert": True},
        # Second best: 98.0% accuracy
        {"psm_mode": 4, "threshold": 128, "scale_factor": 4.0, "border_size": 5, "invert": True},
        # Good with no threshold: 97.1% accuracy
        {"psm_mode": 3, "threshold": None, "scale_factor": 3.0, "border_size": 5, "invert": True},
        # Try PSM 6 with 4x scaling
        {"psm_mode": 6, "threshold": 128, "scale_factor": 4.0, "border_size": 5, "invert": True},
        # Try PSM 11 (sparse text) with 4x scaling
        {"psm_mode": 11, "threshold": 128, "scale_factor": 4.0, "border_size": 5, "invert": True},
    ]
    ...
    for i, config in enumerate(best_configs, 1):
        print(f"\nConfiguration {i}:")
        print(f"  PSM={config['psm_mode']}, Threshold={config.get('threshold', 'None')}, "
